...
import os from 'os'; // Importar OS para detectar plataforma
...
const checkScheduledMessages = async () => {
    try {
        ...
        const { data: messagesToProcess, error } = await supabaseService
        ...
        if (messagesToProcess && messagesToProcess.length > 0) {
            console.log(`[Scheduler] Processing ${messagesToProcess.length} messages...`);
            ...
            for (const msg of messagesToProcess) {
                ...
                if (client) {
                    try {
                        // Attempt to send
                        const sent = await client.sendMessage(msg.chat_id, msg.body);
                        ...
                        console.log(`[Scheduler] Message sent for user ${userId} to ${msg.chat_id}`);
                        
                        // Notify Frontend to update UI
                        const socketId = userSockets.get(userId);
                        if(socketId) {
                            // Re-fetch pending scheduled messages for this chat
                            fetchScheduledMessages(userId, msg.chat_id, socketId);
                        }

                    } catch (sendError) {
                        console.error(`[Scheduler] Failed to send message ${msg.id}:`, sendError.message);
                        // Mark as failed or leave pending? Let's mark as failed to avoid loops
                        await supabaseService
                            .from('scheduled_messages')
                    ...
                    console.log(`[Scheduler] Client not ready for user ${userId}, skipping message ${msg.id}`);
                }
            }
        }
    } catch (e) {
        console.error('[Scheduler] Error checking messages:', e.message);
    }
};
...
async function fetchScheduledMessages(userId, chatId, socketId) {
    try {
        ...
        io.to(socketId).emit('scheduled-messages', { chatId, messages: data || [] });
    } catch(e) {
...
io.on('connection', (socket) => {
    ...
    socket.on('client-ready', () => {
        console.log(`✅ Client ready signal received from user ${userId}`);
        initializeClientForUser(userId, socket.id);
    });
    
    // --- NUEVO: Reset Session Handler ---
    socket.on('reset-session', async () => {
        console.log(`[Session Reset] User ${userId} requested session reset.`);
        try {
            await destroyClientForUser(userId);
            console.log(`[Session Reset] Session destroyed for user ${userId}. Re-initializing...`);
            // Re-initialize immediately to generate new QR
            initializeClientForUser(userId, socket.id);
        } catch(e) {
            console.error(`[Session Reset] Error resetting session for user ${userId}:`, e);
        }
    });
    ...
    socket.on('get-scheduled-messages', async (chatId) => {
        fetchScheduledMessages(userId, chatId, socket.id);
    });

    socket.on('toggle-schedule', async ({ id, active, chatId }) => {
        await supabaseService.from('scheduled_messages').update({ is_active: active }).eq('id', id).eq('user_id', userId);
        fetchScheduledMessages(userId, chatId, socket.id);
    });

    socket.on('delete-schedule', async ({ id, chatId }) => {
        await supabaseService.from('scheduled_messages').delete().eq('id', id).eq('user_id', userId);
        fetchScheduledMessages(userId, chatId, socket.id);
    });
    ...
    socket.on('send-message', async (data) => {
        ...
        try {
            const { to, text, audioBase64, audioMime, isVoiceMessage, scheduledTime } = data;

            // --- HANDLING SCHEDULED MESSAGES ---
            if (scheduledTime) {
                console.log(`[Schedule] Scheduling message for ${to} at ${new Date(scheduledTime * 1000).toISOString()}`);
                const { error } = await supabaseService.from('scheduled_messages').insert({
                    user_id: userId,
                    chat_id: to,
                    body: text,
                ...
                // Confirm and refresh UI
                socket.emit('message-sent-confirmation', { messageId: 'SCHEDULED', timestamp: Date.now() / 1000 });
                fetchScheduledMessages(userId, to, socket.id);
                return; // Stop here, do not send via whatsapp client yet
            }
...
async function initializeClientForUser(userId, socketId) {
    if (clients.has(userId)) {
        const client = clients.get(userId);
        console.log(`[Init] Checking existing client for user ${userId}`);
        try {
            const state = await client.getState();
            if (state === 'CONNECTED') {
                console.log(`[Init] Client ${userId} is CONNECTED`);
                io.to(socketId).emit('connected', true);
                syncAndEmitChats(userId, client);
            } else {
                console.log(`[Init] Client ${userId} state: ${state}`);
                io.to(socketId).emit('connected', false);
            }
        } catch (e) {
            console.error(`[Init] Error checking state for ${userId}, removing client.`);
            clients.delete(userId);
        }
        if (clients.has(userId)) return;
    }
    
    console.log(`[Init] Creating new client instance for user ${userId}`);
    
    // --- Configuración de Puppeteer Adaptable ---
    const isWindows = os.platform() === 'win32';
    const puppeteerArgs = isWindows 
        ? ['--no-sandbox', '--disable-setuid-sandbox'] // Mínimo necesario para Windows
        : [
            '--no-sandbox',
    ...
    const client = new Client({
        ...
        // ELIMINADO webVersionCache para evitar bloqueos en Windows con versiones remotas
        // Se usará la versión por defecto de la librería.
    });
    ...
    client.on('ready', async () => {
        console.log(`[Ready] WhatsApp client ready for user ${userId}`);
        const userSocketId = userSockets.get(userId);
        if (userSocketId) {
    ...
    client.on('auth_failure', (msg) => {
        console.error(`[Auth] Authentication failure for user ${userId}:`, msg);
    });
    ...
    client.on('disconnected', (reason) => {
        console.log(`[Disconnect] Client for user ${userId} disconnected: ${reason}`);
        const userSocketId = userSockets.get(userId);
        if (userSocketId) io.to(userSocketId).emit('connected', false);
    });
    clients.set(userId, client);
    try {
        console.log(`[Init] Calling client.initialize() for user ${userId}`);
        await client.initialize();
    } catch (error) {
        console.error(`[Init] Failed to initialize client for user ${userId}:`, error);
        clients.delete(userId);
    }
}
...
async function destroyClientForUser(userId) {
    const client = clients.get(userId);
    if (!client) {
         // Even if client obj doesn't exist, try to clean files
